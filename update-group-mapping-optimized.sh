#!/bin/bash
# Optimized script to update group mapping for Rocky Linux joined to UCS domain
...
echo "Setting up connection to $ldap_master..."
$SSH_CMD $SSH_OPTS -M -N root@$ldap_master &
SSH_PID=$!
...
mkdir -p /tmp/ucs_sync_helper

# Create a helper script on the UCS server to get group information
echo "Creating helper script on UCS server..."
cat > /tmp/ucs_sync_helper/get_groups.sh << 'EOF'
#!/bin/bash
# Helper script to get group information from UCS server

# Parameters
LDAP_BASE="$1"

# Get all posix groups
ldapsearch -x -H ldap://localhost:7389 -b "$LDAP_BASE" "(objectClass=posixGroup)" dn cn gidNumber > /tmp/posix_groups.ldif

# Format the output for easier parsing
awk '
BEGIN { dn=""; cn=""; gid=""; }
/^dn: / { if (dn != "") { print dn "," cn "," gid; } dn=$0; cn=""; gid=""; }
/^cn: / { cn=$0; }
/^gidNumber: / { gid=$0; }
END { if (dn != "") { print dn "," cn "," gid; } }
' /tmp/posix_groups.ldif > /tmp/formatted_groups.txt

# Clean up
rm -f /tmp/posix_groups.ldif

# Output the formatted groups
cat /tmp/formatted_groups.txt
rm -f /tmp/formatted_groups.txt
EOF

# Copy the helper script to the UCS server
$SSH_CMD root@$ldap_master "cat > /tmp/get_groups.sh" < /tmp/ucs_sync_helper/get_groups.sh
$SSH_CMD root@$ldap_master "chmod +x /tmp/get_groups.sh"

# Execute the helper script on the UCS server
echo "Retrieving group information from UCS server..."
$SSH_CMD root@$ldap_master "/tmp/get_groups.sh '$ldap_base'" > /tmp/ucs_sync_helper/groups.txt

# Create a script to synchronize group memberships
...
# Create a helper script on the UCS server
cat > \$TMP_DIR/get_groups.sh << 'EOFINNER'
#!/bin/bash
# Helper script to get group information from UCS server
...
# Copy the helper script to the UCS server
\$SSH_CMD root@\$ldap_master "cat > /tmp/get_groups.sh" < \$TMP_DIR/get_groups.sh
\$SSH_CMD root@\$ldap_master "chmod +x /tmp/get_groups.sh"

# Execute the helper script on the UCS server
echo "Retrieving group information from UCS server..."
\$SSH_CMD root@\$ldap_master "/tmp/get_groups.sh '\$ldap_base'" > \$TMP_DIR/groups.txt

# Process each group
while IFS=',' read -r dn cn gid; do
    # Extract values
    dn_val=\${dn#dn: }
    cn_val=\${cn#cn: }
...
done < \$TMP_DIR/groups.txt

# Clean up
\$SSH_CMD -O exit root@\$ldap_master
kill \$SSH_PID 2>/dev/null
rm -rf \$TMP_DIR
\$SSH_CMD root@\$ldap_master "rm -f /tmp/get_groups.sh" || true
...
# Process each group from the retrieved data
...
done < /tmp/ucs_sync_helper/groups.txt
